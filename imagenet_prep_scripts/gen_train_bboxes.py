...
if __name__ == '__main__':
  ...
  for file_index, one_file in enumerate(xml_files):
    ...
    for bbox in bboxes:
      ...
      out_file.write('%s.JPEG,%.4f,%.4f,%.4f,%.4f\n' % (image_filename,
             bbox.xmin_scaled, bbox.ymin_scaled, bbox.xmax_scaled, bbox.ymax_scaled))
    ...
    if not file_index % 5000:
      print('--> processed %d of %d XML files.' % (file_index + 1, len(xml_files)))
      print('--> skipped %d boxes and %d XML files.' % (skipped_boxes, skipped_files))
