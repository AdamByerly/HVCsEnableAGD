...
def make_tower(x_in, y_out, is_training, count_classes):
    ...
    with tf.name_scope("caps"):
        ...
        pcap12     = make_caps_from_conv("pcap",
                        caps_input, cap_dims, cap_count)
        ocap12     = make_homogeneous_vector_caps("outcaps",
                        pcap12, count_classes, cap_dims, weight_decay=0.0)
        bn12       = make_batch_norm("bn12", ocap12, is_training)
        relu12     = make_relu("relu12", bn12)
        logits     = make_norm("norm12", relu12)
